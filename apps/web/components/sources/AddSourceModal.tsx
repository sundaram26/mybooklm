...
export function AddSourceModal({ isOpen, onClose, notebookId, onSourceAdded }: AddSourceModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (tab === "file") {
        ...
        if (!url.trim()) return;
        await api.uploadUrl(notebookId, url.trim(), urlTitle.trim() || undefined);
      } else if (tab === "text") {
        if (!text.trim() || !textTitle.trim()) return;
        await api.createTextNote(notebookId, text.trim(), textTitle.trim());
      }
      onSourceAdded();
      onClose();
      // Reset form
